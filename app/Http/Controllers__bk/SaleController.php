...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if($request->payment_status == 2){
            $previous_due_update = Customer::find($request->customer_id)->opening_due + $request->grand_total;
            Customer::where('id',$request->customer_id)->update([
                'opening_due' => $previous_due_update
            ]);
        }
    ...
    public function limsProductSearch(Request $request)
    {
        ...
        elseif($no_discount)
            if($request->src_use != "pos"){
                ...
                $g_type = Customer::find($request->customer_id)->customer_group_id;
                if($g_type == 1){
                    $product[] = $lims_product_data->price;
                }else{
                    $product[] = $lims_product_data->wholesale_price;
    ...
    public function addPayment(Request $request)
    {
        ...
        if($lims_sale_data->payment_status == 2){
            $update_due_amount = $lims_customer_data->opening_due - $request->amount;
            Customer::where('id',$lims_sale_data->customer_id)->update([

                'opening_due' => $update_due_amount
            ]);
        }
