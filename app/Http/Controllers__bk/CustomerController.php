...
class CustomerController extends Controller
{
    ...
    public function customer_due_payment(Request $request){
        $due_update = Customer::find($request->customer_id)->opening_due - $request->amount;
        Customer::where('id',$request->customer_id)->update([
            'opening_due' => $due_update
        ]);

        return redirect()->back()->with('create_message','Due payment successfully');
    }

    public function customer_due_payment_pos(Request $request){
   
        $due_update = Customer::find($request->customer_hidden_id)->opening_due - $request->due_payment_amount;
       
        Customer::where('id',$request->customer_hidden_id)->update([
            'opening_due' => $due_update
        ]);
